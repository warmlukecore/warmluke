...
check(
  "an hour-old access token is not a problem",
  at({ status: "connected", token_expires_at: hourAgo, refresh_token_expires_at: inAMonth }).tone === "ok"
);
check("a store from before expiring tokens works forever", !accessRanOut({ status: "connected" }, NOW));
check(
  "a refresh token that ran out needs a reconnect",
  at({ status: "connected", token_expires_at: hourAgo, refresh_token_expires_at: lastWeek }).reconnect === true
);
check(
  "so does an expiring token with no refresh date",
  accessRanOut({ status: "connected", token_expires_at: hourAgo }, NOW)
);
check("importing says so", at({ status: "connected", importing: true }).tone === "busy");
check(
  "but an import never hides a dead token",
  at({ status: "connected", importing: true, token_expires_at: hourAgo, refresh_token_expires_at: lastWeek }).tone ===
    "warn"
);
check(
  "removed from Shopify needs a reconnect",
  at({ status: "uninstalled" }).reconnect && /Removed/.test(at({ status: "uninstalled" }).label)
);
...
check(
  "an attempt that never came back is not offered as a store",
  /status === "pending" && !s\.connected_at/.test(switcher)
);
...
check(
  "the app shell is mounted fresh for each project",
  /<AppShell key=\{projectId\} projectId=\{projectId\}/.test(src("src/app/app/[projectId]/page.tsx"))
);
check(
  "the switcher sits at the foot of the sidebar",
  /<StoreSwitcher projectId=\{projectId\} placement="sidebar" \/>/.test(src("src/components/AppShell.tsx"))
);
...
check(
  ...
  !isConnectHint("New") && !isConnectHint("new ") && !isConnectHint("../x") && !isConnectHint(null)
);
{
...
check(
  "the connect page makes the project only after Shopify named the store",
  /else if \(wantsNew\) \{\s*started\.current = true;\s*newProject\(\);/.test(connect) &&
    /if \(started\.current \|\| !shop \|\| projects === null \|\| already\) return;/.test(connect)
);
check("and a store already connected is shown, not connected twice", /if \(already\) \{/.test(connect));
check(
  "the switcher's add goes through one tap when there is one, else the dashboard",

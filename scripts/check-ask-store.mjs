...
try {
  const store = must(
    await admin
      ...
      .insert({
        ...
        shop_domain: `ask-${stamp}.myshopify.com`,
        status: "connected",
        currency: "INR",
  ...
  must(
    await admin
      ...
      .insert({ store_id: store.id, variant_id: v.id, location_name: "Main", available: 3 })
  );
  ...
  const route = (o) => ({
    ...
    kind: "ranking",
    needles: [],
    confidence: { list: 1, kind: 1, window: 1 },
    ms: 0,
    ...o,
  });
  ...
  check(
    "a lookup finds by any of the words",
    s?.rows.length === 1 && s?.rows[0]?.name === "Aarav Singh" && /Aarav/.test(s.what)
  );
  ...
  check(
    "customers over a span are that span's orders, with names, to rank from",
    s?.rows.length === 3 &&
      s?.rows[0]?.order_number === "#3" &&
      "customer_name" in s.rows[0] &&
      /rank or count customers/.test(s.what)
  );
  s = await S({ list: "orders", kind: "total", window: "this_week" });
  check(
    "orders in the last seven days, and not the one from forty days ago",
    s?.rows.length === 3 && s?.total === 3 && /last 7 days/.test(s.what)
  );
  s = await S({ list: "orders", kind: "ranking", window: "all" });
  check(
    "orders ranked: biggest first, cancelled one included and marked",
    s?.rows[0]?.order_number === "#4" && s?.rows[0]?.status === "Cancelled"
  );
  s = await S({ list: "sales", kind: "ranking", window: "this_month" });
  check(
    "sales in a span go through the function: the cancelled ten do not count",
    s?.rows[0]?.title === "Ski Wax" &&
      s?.rows[0]?.units === 5 &&
      s?.rows.find((r) => r.title === "Snowboard")?.units === 1
  );
  ...
  if (!keyFor(process.env.TYPESAFE_API_KEY)) {
    ...
    check(
      "a question brings its rows into the snapshot",
      asked?.snapshot?.slice?.read_as?.list === "customers" &&
        asked?.snapshot?.slice?.rows?.[0]?.name === "Bhavna Mehta"
    );
    if (!asked?.snapshot?.slice) show(asked?.snapshot);
    const build = await storeContextFor(
      client,
      project.id,
      "make me a returns section with a reason and refund amount"
    );
    check("a build request brings nothing extra", build?.snapshot?.slice === undefined);
    check(
      "and the fixed snapshot is still there either way",
      (build?.snapshot?.top_customers?.length ?? 0) > 0 && (asked?.snapshot?.recent?.length ?? 0) > 0
    );
    ...
    const tool = async (name, args) => {
      const res = await fetch(`${APP}/api/mcp`, {
        ...
        headers: {
          ...
          Authorization: `Bearer ${me.session.access_token}`,
        },
        body: JSON.stringify({
          jsonrpc: "2.0",
          id: 1,
      ...
      if (res.ok && res.headers.get("x-model-tape") !== process.env.MODEL_TAPE) {
        throw new Error(
          `the server at ${APP} is ${res.headers.get("x-model-tape") ? `in ${res.headers.get("x-model-tape")} mode` : "calling the real router"}, and this check is in ${process.env.MODEL_TAPE} mode; start it with MODEL_TAPE=${process.env.MODEL_TAPE}`
        );
      }
    ...
    check(
      "ask_store answers a ranking with the rows",
      top?.read_as?.list === "customers" && top?.rows?.[0]?.name === "Bhavna Mehta"
    );
    if (!top?.rows) show(top);
    const one = await tool("ask_store", { question: "is #2 paid?" });
    check(
      "and a lookup with the one row",
      one?.read_as?.kind === "lookup" && one?.rows?.length === 1 && one?.rows?.[0]?.order_number === "#2"
    );
    ...
    check(
      "a build request is not routed, and says which tools to use",
      no?.could_not_route === true && /search_orders/.test(no?.note ?? "")
    );

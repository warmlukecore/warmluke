...
try {
  ...
  const { data: written } = await admin
    .from("messages")
    .insert([
      { conversation_id: thread.id, role: "user", content: "first", payload: { kind: "asked", text: "first" }, created_at: at(0) },
      { conversation_id: thread.id, role: "assistant", content: "answer one", payload: { type: "applied", message: "answer one" }, created_at: at(1) },
      { conversation_id: thread.id, role: "user", content: "teh wrong one", payload: { kind: "asked", text: "teh wrong one" }, created_at: at(2) },
      { conversation_id: thread.id, role: "assistant", content: "answer two", payload: { type: "applied", message: "answer two" }, created_at: at(3) },
    ])
  ...
  const marked = async () =>
    Object.fromEntries(
      (
        await admin.from("messages").select("content, payload").eq("conversation_id", thread.id)
      ).data.map((m) => [m.content, m.payload?.superseded === true])
    );

  console.log("before the edit, nothing is retired");
  const before = await marked();
  check("the thread reads as written", Object.values(before).every((v) => v === false));

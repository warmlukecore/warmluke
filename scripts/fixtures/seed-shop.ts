...
const PRODUCTS = [
  {
    title: "Cotton Kurta",
    type: "Apparel",
    vendor: "Chanderi Looms",
    tags: ["summer"],
    status: "ACTIVE",
    variants: [
      { title: "S", price: 899, cost: 420, stock: 14 },
      { title: "M", price: 899, cost: 420, stock: 3 },
    ],
  },
  {
    title: "Silk Saree",
    type: "Apparel",
    vendor: "Kanchi Weaves",
    tags: ["festive", "silk"],
    status: "ACTIVE",
  ...
  },
  {
    ...
    type: "Footwear",
    vendor: "Kolhapur Craft",
    tags: [],
    status: "ACTIVE",
    variants: [
      { title: "7", price: 1299, cost: 610, stock: 22 },
      { title: "8", price: 1299, cost: 610, stock: 9 },
    ],
  },
  {
    title: "Brass Diya Set",
    type: "Home",
    vendor: "Moradabad Metal",
    tags: ["festive"],
    status: "ACTIVE",
...
const VARIANTS = PRODUCTS.flatMap((p, pi) =>
  p.variants.map((v, vi) => ({
    ...v,
    product: pi,
    id: gid("ProductVariant", 81001 + pi * 10 + vi),
    sku: `SS-${101 + pi * 10 + vi}`,
  }))
);
...
const ORDERS: Array<{
  ...
  lines: Array<[variant: number, qty: number]>;
  pay: Pay;
  gateway: string;
  shipped: "DELIVERED" | "IN_TRANSIT" | null;
  cancelled?: true;
  code?: string;
}> = [
  {
...
export function seedNodes(now = Date.now()): Record<Resource, unknown[]> {
  ...
  const refunded = (oi: number) =>
    ORDERS[oi].pay === "REFUNDED"
      ? totals[oi].total
      : ORDERS[oi].pay === "PARTIALLY_REFUNDED"
        ? VARIANTS[ORDERS[oi].lines[0][0]].price
        : 0;
  ...
  const products: GqlProduct[] = PRODUCTS.map((p, pi) => ({
    ...
    variants: {
      nodes: VARIANTS.filter((v) => v.product === pi).map((v, k) => ({
        ...
        title: v.title,
        sku: v.sku,
        barcode: `890${String(1000 + pi * 10 + k).padStart(10, "0")}`,
        price: v.price.toFixed(2),
        updatedAt: at(30),
        inventoryItem: {
          id: gid("InventoryItem", 82001 + pi * 10 + k),
          tracked: true,
          unitCost: { amount: v.cost.toFixed(2) },
        },
      })),
    },
  }));
  ...
  const collections: GqlCollection[] = [
    {
      id: gid("Collection", 89001),
      title: "Festive picks",
      handle: "festive-picks",
      sortOrder: "MANUAL",
      updatedAt: at(30),
      productsCount: { count: 2 },
      products: { nodes: [{ id: productId(1) }, { id: productId(3) }] },
    },
  ];
  ...
  const customers: GqlCustomer[] = CUSTOMERS.map((c, ci) => {
    ...
    return {
      ...
      amountSpent: {
        amount: mine.reduce((s, { oi }) => s + totals[oi].total - refunded(oi), 0).toFixed(2),
        currencyCode: SEED_CURRENCY,
      },
  ...
  const orders: GqlOrder[] = ORDERS.map((o, oi) => {
    ...
    return {
      id: orderId(oi),
      name: `#${1001 + oi}`,
      createdAt: at(o.daysBack),
      updatedAt: at(o.daysBack, 2),
      cancelledAt: o.cancelled ? at(o.daysBack, 1) : null,
      tags: o.pay === "PENDING" ? ["cod"] : [],
      displayFinancialStatus: o.pay,
      displayFulfillmentStatus: o.shipped ? "FULFILLED" : "UNFULFILLED",
      totalPriceSet: inr(t.total),
      currentTotalPriceSet: inr(o.cancelled ? 0 : t.total - back),
      currentSubtotalPriceSet: amt(t.subtotal),
      currentTotalTaxSet: amt(t.tax),
      currentTotalDiscountsSet: amt(t.discount),
      totalShippingPriceSet: amt(SHIPPING),
      customer: o.customer === null ? null : { id: customerId(o.customer) },
      paymentGatewayNames: [o.gateway],
      discountCodes: o.code ? [o.code] : [],
      shippingAddress: { city: c?.city ?? "Pune", provinceCode: c ? "DL" : "MH", countryCode: "IN" },
      lineItems: {
        nodes: o.lines.map(([v, qty], li) => ({
          id: lineId(oi, li),
          title: PRODUCTS[VARIANTS[v].product].title,
          variantTitle: VARIANTS[v].title === "Default Title" ? null : VARIANTS[v].title,
          quantity: qty,
          sku: VARIANTS[v].sku,
          variant: { id: VARIANTS[v].id },
          product: { id: productId(VARIANTS[v].product) },
          originalUnitPriceSet: amt(VARIANTS[v].price),
        })),
      },
      refunds: back
        ? [{ id: gid("Refund", 86001 + oi), createdAt: at(o.daysBack - 2), totalRefundedSet: amt(back) }]
        : [],
      transactions: [
        {
          id: gid("OrderTransaction", 87001 + oi * 10),
          kind: "SALE",
          status: o.pay === "PENDING" ? "PENDING" : "SUCCESS",
          gateway: o.gateway,
          processedAt: at(o.daysBack),
          test: false,
          amountSet: inr(t.total),
        },
        ...(back
          ? [
              {
                id: gid("OrderTransaction", 87001 + oi * 10 + 1),
                kind: "REFUND",
                status: "SUCCESS",
                gateway: o.gateway,
                processedAt: at(o.daysBack - 2),
                test: false,
                amountSet: inr(back),
              },
            ]
        ...
        ...(oi === 0
          ? [
              {
                id: gid("OrderTransaction", 87001 + 9),
                kind: "AUTHORIZATION",
                status: "SUCCESS",
                gateway: "bogus",
                processedAt: at(o.daysBack),
  ...
  const refunds: GqlRefundedOrder[] = orders.map((o, oi) => ({
    ...
    refunds: o.refunds.map((r) => ({
      ...r,
      refundLineItems: { nodes: [{ quantity: ORDERS[oi].pay === "REFUNDED" ? ORDERS[oi].lines[0][1] : 1 }] },
    })),
  }));
  ...
  const fulfillments: GqlFulfilledOrder[] = ORDERS.map((o, oi) => ({
    ...
    fulfillments: o.shipped
      ? [
          {
            id: gid("Fulfillment", 88501 + oi),
            status: "SUCCESS",
            displayStatus: o.shipped,
            createdAt: at(o.daysBack, 20),
            updatedAt: at(Math.max(o.daysBack - 3, 0), 4),
            deliveredAt: o.shipped === "DELIVERED" ? at(o.daysBack - 3) : null,
            trackingInfo: [
              {
                company: "Delhivery",
                number: `DLV${900001 + oi}`,
                url: `https://www.delhivery.com/track/package/DLV${900001 + oi}`,
              },
            ],
          },
        ]
  ...
  const line = (oi: number, li: number) => {
    ...
    return {
      id: lineId(oi, li),
      title: PRODUCTS[v.product].title,
      sku: v.sku,
      variant: { id: v.id },
      product: { id: productId(v.product) },
    };
  };
        ...
        nodes: [
          {
            ...
            returnLineItems: {
              nodes: [
                {
                  id: gid("ReturnLineItem", 89601),
                  quantity: 1,
                  refundedQuantity: 1,
                  returnReasonNote: "Pinches at the heel",
  ...
  const locations: GqlLocation[] = LOCATIONS.map((l, li) => ({
    ...
    name: l.name,
    isActive: true,
    fulfillsOnlineOrders: l.online,
    address: {
      address1: `${12 + li} Market Road`,
      city: l.city,
      province: l.province,
      provinceCode: l.code,
      country: "India",
        ...
        nodes: [
          {
            ...
            quantities: [
              { name: "available", quantity: v.stock },
              { name: "on_hand", quantity: v.stock + (vi % 3) },
              { name: "committed", quantity: vi % 3 },
              { name: "incoming", quantity: v.stock === 0 ? 10 : 0 },
            ],
          },
          ...(vi % 2 === 0
            ? [
  ...
  const drafts: GqlDraftOrder[] = [
    {
      ...
      updatedAt: at(2, 1),
      completedAt: null,
      invoiceUrl: `https://${SEED_SHOP}/invoices/d1`,
      totalPriceSet: inr(3 * 449 + SHIPPING),
      subtotalPriceSet: amt(3 * 449),
      totalTaxSet: amt(0),
      totalShippingPriceSet: amt(SHIPPING),
      customer: { id: customerId(3), displayName: CUSTOMERS[3].name, email: CUSTOMERS[3].email },
      order: null,
      lineItems: {
        nodes: [
          {
            id: gid("DraftOrderLineItem", 90101),
            title: "Brass Diya Set",
            sku: diya.sku,
            quantity: 3,
            variant: { id: diya.id },
            product: { id: productId(diya.product) },
            originalUnitPriceSet: amt(diya.price),
            discountedUnitPriceSet: amt(449),
          },
        ],
      },
    },
    {
      id: gid("DraftOrder", 90002),
      name: "#D2",
      status: "COMPLETED",
      ...
      completedAt: at(27),
      invoiceUrl: null,
      totalPriceSet: inr(totals[0].total),
      subtotalPriceSet: amt(totals[0].subtotal),
      totalTaxSet: amt(totals[0].tax),
      totalShippingPriceSet: amt(SHIPPING),
      customer: { id: customerId(0), displayName: CUSTOMERS[0].name, email: CUSTOMERS[0].email },
      order: { id: orderId(0) },
      lineItems: {
        nodes: ORDERS[0].lines.map(([v, qty], li) => ({
          id: gid("DraftOrderLineItem", 90102 + li),
          title: PRODUCTS[VARIANTS[v].product].title,
          sku: VARIANTS[v].sku,
          quantity: qty,
          variant: { id: VARIANTS[v].id },
          product: { id: productId(VARIANTS[v].product) },
          originalUnitPriceSet: amt(VARIANTS[v].price),
          discountedUnitPriceSet: amt(VARIANTS[v].price),
        })),
      },
    },
  ];
  ...
  const discounts: GqlDiscount[] = [
    {
      ...
      discount: {
        ...
        asyncUsageCount: 2,
        codes: { nodes: [{ code: "DIWALI10" }] },
        customerGets: { value: { __typename: "DiscountPercentage", percentage: 0.1 } },
      },
    },
    {
      id: gid("DiscountAutomaticNode", 91002),
      discount: {
        __typename: "DiscountAutomaticBasic",
        title: "₹100 off over ₹1,999",
        status: "EXPIRED",
        summary: "₹100 off orders over ₹1,999",
        startsAt: at(60),
        endsAt: at(31),
        createdAt: at(60),
        usageLimit: null,
        ...
        codes: null,
        customerGets: {
          value: { __typename: "DiscountAmount", amount: { amount: "100.00", currencyCode: SEED_CURRENCY } },
        },
      },
    },
  ];
  ...
  const carts: GqlCart[] = [
    {
      id: gid("AbandonedCheckout", 92001),
      abandonedCheckoutUrl: `https://${SEED_SHOP}/checkouts/c1/recover`,
      createdAt: at(1, 3),
      updatedAt: at(1, 4),
      totalPriceSet: inr(2499 + SHIPPING),
      customer: { id: customerId(1), displayName: CUSTOMERS[1].name, email: CUSTOMERS[1].email },
      lineItems: { nodes: [{ title: "Silk Saree", quantity: 1 }] },
    },
    {
      id: gid("AbandonedCheckout", 92002),
      abandonedCheckoutUrl: null,
      createdAt: at(6),
      updatedAt: null,
      totalPriceSet: inr(2 * 349 + SHIPPING),
      customer: null,
      lineItems: { nodes: [{ title: "Jute Tote Bag", quantity: 2 }] },
    },
  ];
  ...
  const payouts: GqlPayout[] = [
    {
      id: gid("ShopifyPaymentsPayout", 93001),
      status: "PAID",
      transactionType: "DEPOSIT",
      issuedAt: at(14),
      net: { amount: "4210.00", currencyCode: SEED_CURRENCY },
      summary: {
        chargesGross: sum(4390),
        chargesFee: sum(180),
        refundsFeeGross: sum(0),
        refundsFee: sum(0),
        adjustmentsGross: sum(0),
        adjustmentsFee: sum(0),
        reservedFundsGross: sum(0),
        reservedFundsFee: sum(0),
        retriedPayoutsGross: sum(0),
        retriedPayoutsFee: sum(0),
        advanceGross: sum(0),
        advanceFees: sum(0),
      },
    },
    {
      id: gid("ShopifyPaymentsPayout", 93002),
      status: "SCHEDULED",
      transactionType: "DEPOSIT",
      issuedAt: at(-1),
      net: { amount: "3105.50", currencyCode: SEED_CURRENCY },
      summary: null,
    },
  ];

-- Migration 0081: what a build wrote, so an undo can find it.
--
-- Two kinds of build could not be put back, for want of one fact each.
-- records_insert answered with a count, and "the five rows this added"
-- cannot be deleted from a count. module_update answered with a count,
-- and a section's old name and place were never written down.
--
-- Both now answer with the fact: the ids of the rows made, and the
-- name, icon, parent and position the section had before — read in
-- the same statement as the change, so nothing can slip in between.
--
-- Callers: src/lib/apply.ts (RECORD_SEED, MODULE_UPDATE, putBack).

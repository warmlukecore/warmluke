...
check(
  "every hero has a headline",
  HEROES.every((h) => h.headline?.length > 0)
);
check(
  "and something to click",
  HEROES.every((h) => h.cta?.length > 0)
);
check("ids are unique", new Set(HEROES.map((h) => h.id)).size === HEROES.length);
check("the default is one of them", !!heroById(DEFAULT_HERO));
check("the live weights add up to a hundred", HEROES.filter((h) => h.live).reduce((n, h) => n + h.weight, 0) === 100);
check(
...
check("a campaign continues into its hero", resolveHero({ utmCampaign: "replace_apps" }).hero.id === "apps");
check(
...
check("a reload keeps what they were given", resolveHero({ assigned: "problem", random: 0 }).hero.id === "problem");
check("but an ad still overrides it", resolveHero({ assigned: "problem", wlVariant: "chatgpt" }).hero.id === "chatgpt");
check(
...
check(
  ...
  Array.from({ length: 400 }, (_, i) => assignHero(i / 400).id).every((id) => heroById(id)?.live === true)
);
...
for (const h of HEROES) {
  ...
  if (h.emphasis) {
    check(
      `${h.id}: "${h.emphasis}" is set in italic`,
      parts.some((p) => p.kind === "italic" && p.text === h.emphasis)
    );
  }
  if (h.cycle) {
    check(
      `${h.id}: "${h.cycle.word}" is the word that cycles`,
      parts.some((p) => p.kind === "cycle" && p.text === h.cycle.word)
    );
...
check(
  "a variant with no markers renders whole",
  headlineParts(plain)
    .map((p) => p.text)
    .join("") === "Just a line."
);
...
check(
  "a marker that is not there loses no words",
  headlineParts(wrong)
    .map((p) => p.text)
    .join("") === "Just a line."
);
check(
  "and marks nothing",
  headlineParts(wrong).every((p) => p.kind === "plain")
);
const both = {
  ...plain,
  headline: "One two three.",
  emphasis: "three",
  cycle: { word: "One", through: ["One", "Two"] },
};
...
const clash = {
  ...
  headline: "One two three.",
  emphasis: "One two",
  cycle: { word: "two", through: ["two", "three"] },
};
check(
  "overlapping markers do not duplicate a word",
  headlineParts(clash)
    .map((p) => p.text)
    .join("") === "One two three."
);
...
for (const n of [2, 3, 5, 9]) {
  ...
  check(
    `${n} names: every keyframe sits inside the loop`,
    pcs.every((v) => v >= 0 && v <= 100)
  );
  check(
    `${n} names: they only ever move forwards`,
    pcs.every((v, i) => i === 0 || v >= pcs[i - 1])
  );
...
// page saying something about its own store that is not so.
{
  ...
  check(
    "orders are newest first, each number once",
    numbers.every((n, i) => i === 0 || numbers[i - 1] - n === 1)
  );
  check(
  ...
  check(
    "nothing unpaid has been sent",
    ORDERS.every((o) => o.payment !== "pending" || !o.sent)
  );
  check(
    "something is out of stock, so the overview has stock to watch",
    OUT.length > 0 && OUT.every((v) => v.stock <= 0)
  );
  check(
    "the low list is exactly what is under the line",
    LOW.length > 0 && LOW.length === VARIANTS.filter((v) => v.stock < LOW_STOCK).length
  );
  check(
  ...
  check(
    "the customer Luke finds really is waiting on a payment",
    FOLLOW_UP.orders[0].payment === "pending" && FOLLOW_UP.orders.length > 1
  );
  check("every day of the week Luke draws had orders", FIGURES.week.length === 7 && FIGURES.week.every((n) => n > 0));
  check(
    "every return is an order that exists, refunded when it says so",
    RETURNS.every((r) => ORDERS.includes(r.order) && (r.stage === "Refunded") === (r.order.payment === "refunded"))
  );
  check(
    "every order belongs to a customer on the list",
    CUSTOMERS.reduce((s, c) => s + c.orders.length, 0) === ORDERS.length
  );
}
...
// submits, and books a lead with that answer silently missing.
{
  ...
  check(
    ...
    ["TEAM_OPTIONS", "ORDER_OPTIONS", "HEARD_OPTIONS"].every((l) =>
      new RegExp(`pick\\(form\\.get\\("[a-z_]+"\\), ${l}\\)`).test(action)
    )
  );
}

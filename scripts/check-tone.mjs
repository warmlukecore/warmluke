...
check("PENDING is a payment waiting", badgeLabel("PENDING") === "Payment pending" && knownStatus("PENDING")?.tone === "attention");
...
check("a status Shopify might add later is its own word, calmly", knownStatus("ON_ICE") === null && badgeLabel("ON_ICE") === "ON_ICE");

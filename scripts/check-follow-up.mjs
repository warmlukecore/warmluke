...
console.log("the file a spreadsheet opens");
{
  ...
  check(
    "a comma, a quote or a new line is quoted",
    one("a,b") === '"a,b"' && one('say "hi"') === '"say ""hi"""' && one("a\nb") === '"a\nb"'
  );
...
console.log("\nthe stages are the database's");
{
  const sql = readFileSync(
    new URL("../supabase/migrations/0120_following_up_and_the_whole_story.sql", import.meta.url),
    "utf8"
  );
  ...
  const block = parts.slice(
    parts.indexOf("export const DEMO_STAGES"),
    parts.indexOf("];", parts.indexOf("export const DEMO_STAGES"))
  );
  const screen = [...block.matchAll(/value: "([a-z_]+)"/g)].map((m) => m[1]).join(",");
  const tones = parts.slice(
    parts.indexOf("export const STAGE_TONE"),
    parts.indexOf("};", parts.indexOf("export const STAGE_TONE"))
  );
  ...
  check(
    "and every one has a badge",
    screen.split(",").every((v) => new RegExp(`\\b${v}:`).test(tones))
  );

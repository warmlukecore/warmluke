...
if (env.CHECK_PROJECT !== "1") {
  console.log(
    `${envFile} does not declare CHECK_PROJECT=1 — this makes the check user an administrator of whatever project that file points at, so it refuses`
  );
  process.exit(2);
}
...
const { error } = await admin.from("account_settings").update({ is_superadmin: true }).eq("user_id", me.user.id);
if (error) {
  console.log("could not promote:", error.message);
  process.exit(1);
}
...
if (!seed.session) {
  console.log("no seed account:", seed.why);
  process.exit(1);
}
await admin
  .from("account_settings")
  .upsert({ user_id: seed.user.id }, { onConflict: "user_id", ignoreDuplicates: true });
const gone = await admin.from("projects").delete().eq("owner_id", seed.user.id);
if (gone.error) {
  console.log("could not clear the old seed:", gone.error.message);
  process.exit(1);
}
const { data: project, error: pe } = await admin
  ...
  .single();
if (pe) {
  console.log("could not make the seed project:", pe.message);
  process.exit(1);
}
...
const { data: store, error: se } = await admin
  ...
  .single();
if (se) {
  console.log("could not make the seed store:", se.message);
  process.exit(1);
}
...
if (runs.error) {
  console.log("could not record the seed import:", runs.error.message);
  process.exit(1);
}

...
- `<key>.json`: one request's answers, in the order they came. The key is the request
  ...
  addresses) replaced by placeholders, and without the model's name. `asked` is the last
...
needed them. Answers that stop being used can be deleted: a file no request names is never

...
const store = { id: "s1", project_id: "p1", shop_domain: "bishop.myshopify.com", timezone: "Asia/Kolkata", currency: "INR", last_synced_at: null };
...
check("a change the registry does not know: named, with what can be asked", !unknown.ok && /no change called "delete_everything"/.test(unknown.answer.error) && unknown.answer.what_can_be_asked_for.length > 0);
...
check("a target that is not Shopify's own id is named back", !loose.ok && /cannot be acted on/.test(loose.answer.error) && /"1001" is not a Shopify id/.test(loose.answer.these[0]));
const many = await proposeStoreAction(db(), store, { ...tag, targets: Array.from({ length: MOST_TARGETS + 1 }, (_, i) => ({ id: `gid://shopify/Order/${i + 1}` })) });
check(`more than ${MOST_TARGETS} at once is refused`, !many.ok && new RegExp(`${MOST_TARGETS + 1} things at once`).test(many.answer.error));
const noTag = await proposeStoreAction(db(), store, { ...tag, params: {} });
check("what the change itself needs is checked: a tag with no tag", !noTag.ok && noTag.answer.error === "No tag was given.");
const short = await proposeStoreAction(db({ scopes: ["read_orders"] }), store, tag);
check("a store that has not allowed it: say which, and that it needs reconnecting", !short.ok && short.reconnect === true && /has not allowed Warmluke to write_orders/.test(short.answer.error));
check("and none of those asked for anything", proposed.length === 0);

console.log("\nasked, with the card's words written from the change");
const asked = await proposeStoreAction(db({ scopes: ["write_orders", "write_customers", "write_products"] }), store, tag);
check("it is asked, once", asked.ok && proposed.length === 1 && asked.id === "act-1");
check("the sentence comes from the change, not from the assistant", proposed[0].p_summary === actionSpec("add_tags").say([{ id: order }], { tags: ["VIP"] }) && asked.summary === proposed[0].p_summary);
check("on this store, in this app, with what was named", proposed[0].p_store === "s1" && proposed[0].p_project === "p1" && proposed[0].p_targets[0].id === order);
...
check("a bare id is lifted into a target", readTargets([order]).targets[0].id === order && readTargets([order]).wrong.length === 0);
...
check("the same change twice in a turn is one request", proposed.length === 1 && JSON.stringify(first) === JSON.stringify(again));
check("heard once, with the card's sentence", heard.length === 1 && heard[0].summary === proposed[0].p_summary);
check("and tells the model it is waiting, not done", first.status === "waiting for the merchant" && /Never say it is done/.test(first.note));
const lukeOff = aiProposeTool({ db: db({ on: false }), store });
check("with the switch off it answers the same no", /not turned on/.test((await lukeOff.execute(tag, { toolCallId: "t3", messages: [] })).error));
...
console.log("\nMCP asks the same way");
{
  ...
  check("with no second copy of the gates", !route.includes("function readTargets") && !route.includes("not turned on for this account") && !route.includes('rpc("abo_action_propose"'));
}

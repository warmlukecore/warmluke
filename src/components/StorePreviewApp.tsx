...
// ─────────────────────────────────────────────────────────────
// The glimpse of the app, itself. src/components/StorePreview.tsx holds
// its place with a skeleton until it can be drawn, and decides the size
// it is drawn at.
//
...
// app's tokens. Fed the sample store (src/lib/sample-store.ts) instead of
// a database, so a visitor who signs up opens the screen they were shown.
//
// Only the overview and the sections a merchant starts with can be
// opened. Luke's panel is there as it first appears, and pressing it goes
// to the part of the page that shows Luke working. A link out of the app,
// to the store's Shopify admin, would lead nowhere for a sample store, so
// Luke says that instead.
//
// Callers: src/components/StorePreview.tsx.
// ─────────────────────────────────────────────────────────────
...
/** The glimpse, in the sample store's money and words: what the shell loads. */
export default function GlimpseApp(props: { now: number; narrow: boolean; theme: Theme; onTheme: (t: Theme) => void }) {
  return (

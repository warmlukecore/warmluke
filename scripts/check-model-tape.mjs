...
const saved = { key: process.env.ANTHROPIC_API_KEY, model: process.env.ANTHROPIC_MODEL, url: process.env.ANTHROPIC_API_URL };
const logged = [];
...
const request = (said, system = "the contract", extra = {}) =>
  JSON.stringify({ model: "claude-x", max_tokens: 6000, system: [{ type: "text", text: system }], messages: [{ role: "user", content: said }], ...extra });
const ask = (said, system) => tapeFetch("anthropic", net)("https://api.anthropic.com/v1/messages", { method: "POST", body: request(said, system) });
...
try {
  ...
  check("with no setting, a call goes straight to the network", (await (await ask("hi")).text()) === "straight through" && calls === 1 && readdirSync(tapes).length === 0);
  ...
  check(
    ...
    normalise("3f2b1c4d-1111-2222-3333-444455556666 2026-09-24T12:00:01.5Z 2026-08-25 gid://shopify/Order/31003 luke-mufjhory.myshopify.com toolu_01AbC") ===
      "‹id› ‹time› ‹date› gid://shopify/Order/‹n› ‹shop› ‹toolu›"
  );
  const a = fingerprint("anthropic", "https://api.anthropic.com/v1/messages", request("is #1001 paid? store a-1.myshopify.com, row 3f2b1c4d-1111-2222-3333-444455556666"));
  const b = fingerprint("anthropic", "https://api.atria-asi.ai/v1/messages", request("is #1001 paid? store b-2.myshopify.com, row 9a9a9a9a-1111-2222-3333-444455556666", "the contract", { model: "Atria-Dawn-Preview" }));
  ...
  check("a different question is not", c.key !== fingerprint("anthropic", "https://api.anthropic.com/v1/messages", request("is #1001 paid?")).key);
  const d = fingerprint("anthropic", "https://api.anthropic.com/v1/messages", request("is #1001 paid?", "a changed contract"));
  const e = fingerprint("anthropic", "https://api.anthropic.com/v1/messages", request("is #1001 paid?"));
  check("nor is the same question under a different prompt", d.key !== e.key && d.parts.conversation === e.parts.conversation);
  ...
  check("recording makes the real call and hands back what it said", calls === 2 && r1.status === 429 && (await r2.text()) === '{"ok":1}');
  const files = readdirSync(tapes).filter((f) => !f.startsWith("_"));
  const kept = JSON.parse(readFileSync(join(tapes, files[0]), "utf8"));
  check("into one tape, both answers in order, with what was asked", files.length === 1 && kept.responses.map((r) => r.status).join() === "429,200" && /tag #1003 VIP/.test(kept.asked));
  check("and nothing secret in it: no key, no header", !JSON.stringify(kept).includes("x-api-key") && !JSON.stringify(kept).includes("test-key"));
  process.env.MODEL_TAPE = "replay";
  calls = 0;
  ...
  check("a retry plays back as it happened: busy, then the answer, then the answer again", p1.status === 429 && p2.status === 200 && (await p3.text()) === '{"ok":1}');
  check("with the answer's own type, so a stream is read as one", p2.headers.get("content-type") === "text/event-stream");
  ...
  check("and says the prompt changed, and how to fix it", logged.some((l) => /different system prompt/.test(l) && /MODEL_TAPE=record/.test(l)));
  logged.length = 0;
  await ask("something never asked");
  check("or that nothing like it was recorded", logged.some((l) => /nothing like this conversation was recorded/.test(l)));
  const halt = new AbortController();
  halt.abort();
  const stop = await tapeFetch("anthropic", net)("https://api.anthropic.com/v1/messages", { method: "POST", body: request("tag #1003 VIP"), signal: halt.signal }).catch((err) => err);
  ...
  process.env.MODEL_TAPE = "replay";
  check("replay uses the model the tapes were recorded with", tapedSetting("ANTHROPIC_MODEL", "claude-sonnet-4-5") === "gemini-3.6-flash");
  check("and what was never recorded is left as the environment has it", tapedSetting("ANTHROPIC_GAP_MODEL", "claude-haiku") === "claude-haiku");
  ...
  delete process.env.MODEL_TAPE;
  for (const [k, v] of [["ANTHROPIC_API_KEY", saved.key], ["ANTHROPIC_MODEL", saved.model], ["ANTHROPIC_API_URL", saved.url]]) {

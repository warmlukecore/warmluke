-- Migration 0077: an undo has to know what it is undoing.
--
-- AUTOMATION_ADD does two different things under one name. If a rule
-- with that name is already on that section, automation_insert rewrites
-- it in place and keeps its id, so its run history survives — 0051.
-- Otherwise it makes a new one. The answer was the id either way, and
-- the id is the same shape in both cases.
--
-- So "put it back" could not tell a rule it created from a rule it
-- edited, and treated both as created: it switched the rule off. For an
-- edit that is the wrong answer twice over — the merchant loses the
-- rule they had, and the definition their AI replaced is gone from
-- anywhere the app can reach.
--
-- Two changes. automation_insert now says whether it created the row
-- and, if not, what the row held first. And automation_restore puts one
-- rule back BY ID: automation_disable matches on name, and a name is
-- unique only within a section, so undoing a rule called "Flag overdue"
-- switched off every rule of that name in the app.
--
-- automation_disable is left as it is. AUTOMATION_REMOVE names a rule
-- because the merchant named it, and changing what that means is a
-- different decision from this one.
--
-- Callers: src/lib/apply.ts (AUTOMATION_ADD, putBack).

...
function NavHeading({ text, action }: { text: string; action?: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between px-2 pt-4 pb-1.5 first:pt-1">
      <span className="text-xs font-medium text-frame-fg-muted">{text}</span>
      {action}
    </div>
  );
}
...
export default function AppShell({
  ...
  useEffect(() => {
    ...
    //
    // A store taken off Shopify still has its rows until Shopify asks for
    // them to be erased, so its sections keep showing them. One that never
  ...
  const addCoreSections = useCallback(async () => {
    if (addingCore) return;
    setAddingCore(true);
    const failed = await addStoreSections(
      projectId,
      CORE_STORE_TABLES.filter((t) => !modules.some((m) => m.source_table === t))
    );
    setAddingCore(false);
    await loadModules();
  ...
  useEffect(() => {
    if (storeSynced > 0 && selectedModuleId && isStoreTable(loadedSource)) loadModuleData(selectedModuleId);
    // Only when the store has been read again, not whenever the section changes.

-- Migration 0048: a build that half happened is written down as half.
--
-- Every path marked a request 'built' as soon as one plan landed.
-- "partly built" existed only in the sentence sent back to whoever was
-- watching; the database kept no memory of it. So a request that
-- created a section and then failed to give it any fields is, on the
-- record, finished — and everything built on that record inherits the
-- lie. A history tool would report it done, and an undo would not know
-- which steps to reverse.
--
-- I fixed this in the panel a commit ago and stopped there, which was
-- the wrong place. Three callers write this status and they disagreed
-- with each other; the fix belongs in the one operation all three go
-- through.
--
-- outcome holds what applyPlans actually returned, in order. That is
-- the smallest thing an undo can later be built on: you cannot reverse
-- steps you never recorded.
--
-- Callers: src/app/api/mcp/route.ts, src/components/ChatPanel.tsx.

alter table public.build_requests
  add column if not exists outcome jsonb;

comment on column public.build_requests.outcome is
  'What the build really did: {applied:[…], errors:[…]}, in the order the plans ran.';

alter table public.build_requests
  drop constraint if exists build_requests_status_allowed;
alter table public.build_requests
  add constraint build_requests_status_allowed
  check (status in ('pending', 'opened', 'building', 'dismissed', 'built', 'partly_built'));

-- Reproduced from 0047 with the request_built branch changed; create
-- or replace takes the whole body or none of it.

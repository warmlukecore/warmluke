...
try {
  ...
  check(
    ...
    billing instanceof ModelError &&
      billing.kind === "billing" &&
      /paused.*our side.*Nothing was changed/.test(billing.message)
  );
  check(
    "and the provider's JSON never reaches the sentence",
    !/credit balance|invalid_request_error|\{/.test(billing.message)
  );
  check(
    "but does reach the log, under one prefix",
    logged.some((l) => /^\[model\] anthropic 400 billing: .*credit balance/.test(l))
  );

  const auth = await failing(401, '{"error":{"type":"authentication_error"}}');
  check(
    "a refused key: on our side, not a retry",
    auth?.kind === "auth" && /our side/.test(auth.message) && !isTransient(auth)
  );
  const busy = await failing(429, '{"error":{"type":"rate_limit_error"}}');
  check(
    "busy: try again in a minute, and worth a retry",
    busy?.kind === "busy" && /try again/.test(busy.message) && isTransient(busy)
  );
  ...
  check(
    "a 503 page: could not reach the model, worth a retry",
    down?.kind === "down" && /could not reach/.test(down.message) && isTransient(down)
  );
  ...
  check(
    "an abort passes through untouched, not dressed as an outage",
    stopped?.name === "AbortError" && !(stopped instanceof ModelError)
  );
  ...
  check(
    "says so in a sentence, and is not retried",
    unset?.kind === "unset" && /not set up/.test(unset.message) && !isTransient(unset)
  );
  ...
  check(
    "to the messages URL, on our key",
  ...
  check(
    ...
    req?.body?.system?.length === 2 &&
      req.body.system[0].text === "the contract" &&
      req.body.system[0].cache_control?.type === "ephemeral" &&
      req.body.system[1].text === "this project" &&
      !req.body.system[1].cache_control
  );
  check(
  ...
  const said = await callAnthropicChat(
    ["the contract", "this project"],
    [
  ...
  check(
    "to the model's generateContent, on the Gemini key",
    (g?.url ?? "").endsWith("/v1beta/models/gemini-test:generateContent") &&
  ...
  check(
    ...
    g?.body?.systemInstruction?.parts?.map((x) => x.text).join("") === "the contract\n\nthis project"
  );
  check(
    "and the assistant speaking as the model",
    JSON.stringify(g?.body?.contents?.map((c) => c.role)) === '["user","model","user"]'
  );
  sent.length = 0;
  ...
  check(
    "it looks the order up, then replies from it",
    looked === '{"type":"answer","message":"#1042 is paid: 1499."}' &&
      ran.length === 1 &&
      ran[0].order_number === "1042"
  );
  check(
    "the tool is offered with its schema",
    sent[0]?.body?.tools?.[0]?.name === "get_order" &&
      sent[0].body.tools[0].input_schema?.required?.[0] === "order_number"
  );
  const second = JSON.stringify(sent[1]?.body?.messages ?? []);
  check(
    "and what it found goes back to the model",
    sent.length === 2 && second.includes('"tool_result"') && second.includes("PAID")
  );
  check(
  ...
  const cut = await callModel({
    system: "s",
    turns: [{ role: "user", content: "are #1042 and #1043 paid?" }],
    lookups: lookups(2),
  });
  const fold = sent[2]?.body;
  check(
    "when the cap comes mid-lookup, it is still answered",
    cut === '{"type":"answer","message":"both paid"}' && ran.length === 2
  );
  check(
    "by one more call, without tools",
    sent.length === 3 && !fold?.tools && !JSON.stringify(fold?.messages ?? []).includes("tool_use")
  );
  check(
    "carrying what the lookups found, in words",
    JSON.stringify(fold?.messages ?? []).includes("What your lookups returned") &&
      JSON.stringify(fold?.messages ?? []).includes("1043")
  );
        ...
        candidates: [
          {
            content: {
              role: "model",
              parts: [{ functionCall: { name: "get_order", args: { order_number: "1042" } } }],
            },
            finishReason: "STOP",
            index: 0,
          },
  ...
  check(
    ...
    sent[0]?.body?.tools?.[0]?.functionDeclarations?.[0]?.name === "get_order" &&
  ...
  check(
    "the words so far, mid-string",
    draftMessage('{"type":"answer","message":"Order #1001 is not') === "Order #1001 is not"
  );
  check(
    "the whole message once it closes, and not a word past it",
    draftMessage('{"type":"answer","message":"Paid.","kind":"store"}') === "Paid."
  );
  check(
    "escapes read as what they stand for",
    draftMessage('{"message":"a \\"quote\\"\\nline 2') === 'a "quote"\nline 2'
  );
  check(
    "and a half-arrived escape waits",
    draftMessage('{"message":"cost \\u20') === "cost " && draftMessage('{"message":"₹ is \\u20b9') === "₹ is ₹"
  );
  check(
  ...
  const anthropicStream = (chunks, end = "end_turn") =>
    sse([
      ...
      [
        "message_delta",
        { type: "message_delta", delta: { stop_reason: end, stop_sequence: null }, usage: { output_tokens: 20 } },
      ],
  ...
  const streamed = await callModel({
    system: ["the contract", "this project"],
    turns: [{ role: "user", content: "is #1001 paid?" }],
    onText: (t) => heardText.push(t),
  });
  check("the reply comes back whole, as ever", streamed === '{"type":"answer","message":"Order #1001 is not paid."}');
  check(
    "heard as it grew, starting from nothing",
    heardText[0] === "" && heardText.at(-1) === streamed && heardText.length >= 4
  );
  const drafts = heardText.map(draftMessage).filter((d) => d !== null);
  check(
    "and the draft read from it grows the same way",
    JSON.stringify(drafts) === JSON.stringify(["Order #1001 ", "Order #1001 is not paid."])
  );
  check(
  ...
  check(
    "a stream refused at the door is the same sentence",
    refusedStream instanceof ModelError && refusedStream.kind === "busy"
  );
  ...
  check(
    "and one that breaks off midway, overloaded, is busy too",
    midway instanceof ModelError && midway.kind === "busy"
  );
  ...
  queue.push({
    ...
    body: [
      ...
      {
        candidates: [{ content: { role: "model", parts: [{ text: 'id."}' }] }, finishReason: "STOP", index: 0 }],
  ...
  const gs = await callModel({
    system: "s",
    turns: [{ role: "user", content: "paid?" }],
    model: "gemini-test",
    onText: (t) => geminiHeard.push(t),
  });
  check(
    "Gemini streams too, asked for JSON",
    gs === '{"type":"answer","message":"Paid."}' &&
      /streamGenerateContent/.test(sent[0]?.url ?? "") &&
      sent[0]?.body?.generationConfig?.responseMimeType === "application/json"
  );
  check(

-- Migration 0085: one list of store tables.
--
-- product_sales (0084) is the fifth list a section can be pointed at.
-- The other four were named in two places that had to agree with the
-- code: the check constraint on modules.source_table (0025) and the
-- refusal inside abo_build (0033 onward, last redefined in 0081). A
-- section over product_sales passed the validator, the approval and
-- the build function's every other gate, and failed on the insert
-- with a constraint error nobody had written a message for.
--
-- So: one function, abo_is_store_table, and both places ask it. The
-- next store table is one line here. abo_build is otherwise 0081's,
-- copied by a script rather than retyped.
--
-- Callers: src/lib/apply.ts and src/app/api/mcp/route.ts (abo_build),
-- src/lib/store-read.ts (STORE_TABLES is the same list, in code).

create or replace function public.abo_is_store_table(t text) returns boolean
language sql immutable as $$
  select t in ('orders', 'customers', 'products', 'inventory_levels', 'product_sales');
$$;
revoke all on function public.abo_is_store_table(text) from public;
grant execute on function public.abo_is_store_table(text) to authenticated;

alter table public.modules
  drop constraint if exists modules_source_table_allowed;
alter table public.modules
  add constraint modules_source_table_allowed
  check (source_table is null or public.abo_is_store_table(source_table));
